...
import {
  ...
  Produktionsteile,
  SpaceAfterRow,
} from "../dtos/Produktionsteile.tsx";
...
export function TeileProduktion() {
  ...
  const produktionsPlan =
    generalStore?.produktionsPlan ?? new ProduktionsPlanDTO();

  const [produktionsteile, setProduktionsteile] = useState<Produktionsteil[]>(
    initializeProduktionsteile()
  );
  ...
  function initializeProduktionsteile() {
    ...
    Produktionsteile.forEach((teil: Produktionsteil) => {
      const produktionsteil = new Produktionsteil(
        teil.id,
        teil.planRestbestand
      );

      //set Restbestand
      if (restBestand !== undefined) {
        produktionsteil.restBestand = restBestand.filter(
          (r) => r.id == produktionsteil.id
        )[0].amount;
      }
      ...
      for (const station of waitingWorkplace) {
        ...
        const listArray = Array.isArray(waitingList)
          ? waitingList
          : [waitingList];
      ...
      const uniqueWarteschlange = warteschlange.filter(
        (item, index, self) =>
          index ===
          self.findIndex((b) => b.item === item.item && b.order === item.order)
      );
      ...
      if (inBearbeitung !== undefined) {
        ...
        const uniqueBearbeitung = Array.isArray(inBearbeitung)
          ? inBearbeitung.filter(
              (item, index, self) =>
                index ===
                self.findIndex(
                  (b) => b.item === item.item && b.order === item.order
                )
            )
          : [inBearbeitung];
    ...
    teil1.planRestbestand =
      Number(produktionsPlan.p1ProduktionWoche0) +
      Number(teil1.restBestand) +
      Number(teil1.warteschlange) +
      Number(teil1.bearbeitung) -
      Number(verkaufsauftraege.p1);
    ...
    teil2.planRestbestand =
      Number(produktionsPlan.p2ProduktionWoche0) +
      Number(teil2.restBestand) +
      Number(teil2.warteschlange) +
      Number(teil2.bearbeitung) -
      Number(verkaufsauftraege.p2);
    ...
    teil3.planRestbestand =
      Number(produktionsPlan.p3ProduktionWoche0) +
      Number(teil3.restBestand) +
      Number(teil3.warteschlange) +
      Number(teil3.bearbeitung) -
      Number(verkaufsauftraege.p3);
  ...
  function setAuftrag(kaufteilNummer: number, planRestbestand: number) {
    setProduktionsteile((prevState) =>
      prevState.map((produktionsAuftrag) => {
        if (produktionsAuftrag.id === kaufteilNummer) {
          // 16, 17 and 26 are used on all bikes
          if (
            produktionsAuftrag.id === 16 ||
            produktionsAuftrag.id === 17 ||
            produktionsAuftrag.id === 26
          ) {
  ...
  function save() {
    const preferedOrder = [
      1, 2, 3, 261, 161, 171, 13, 19, 15, 14, 262, 20, 18, 10, 11, 12, 263, 162,
      172, 7, 8, 9, 49, 54, 29, 4, 5, 6, 50, 55, 30, 51, 56, 31, 163, 173,
    ];
    ...
    const production = produktionsteile
      .filter(
        (auftrag) => auftrag.menge > 0 && ![16, 17, 26].includes(auftrag.id)
      )
              ...
              <>
                <tr key={id + "vorne"} style={{ border: "none" }}>
                  {(id === 2 || id === 3) && (
                    <td
                      colSpan={2}
                      style={{ padding: 0, height: "15px", border: "none" }}
                    />
                  )}
                </tr>
                <tr key={id + "mitte"}>
                  ...
                  <td>
                    - {formatZahl(id, produktionsteile[id].warteschlange)}
                  </td>
                ...
                <tr key={id + "hinten"} style={{ border: "none" }}>
                  {SpaceAfterRow.includes(id) && (
                    <td
                      colSpan={2}
                      style={{ padding: 0, height: "45px", border: "none" }}
                    />
                  )}
                </tr>
              </>
      ...
      {speicherInfo && (
        <div className="text-center mt-3">
          <div className="alert alert-primary" role="alert">
            {t(
              "TeileProduktion.Eigenfertigprodukt Produktionsaufträge wurde erfolgreich gespeichert."
            )}
          </div>
        </div>
      )}
